...
// 포인트 데이터 중심 좌표 (구미 DEM 영역)
let DEM_CENTER = { lon: 128.2, lat: 36.07 };
...
async function toggleOriginalPoints() {
  ...
  try {
    ...
    showNotification(
      `원본 포인트 ON (${originalPointsData.points.length}개)`,
      "success"
    );
    return true;
  } catch (error) {
...
async function toggleGeneratedPoints() {
  ...
  try {
    ...
    showNotification(
      `생성 포인트 ON (${generatedPointsData.points.length}개)`,
      "success"
    );
    return true;
  } catch (error) {
...
function updateCenterFromPoints(points) {
  ...
  console.log(
    `중심 좌표 업데이트: ${DEM_CENTER.lon.toFixed(4)}, ${DEM_CENTER.lat.toFixed(
      4
    )}`
  );
}
...
function setSunAngle(hour) {
  const time = Cesium.JulianDate.fromIso8601(
    `2024-06-21T${String(hour).padStart(2, "0")}:00:00Z`
  );
